...
| # | 시나리오 | 자리 | 결과 |
|---|---|---|---|
| 1 | 정상 거래 (등록→예약→결제→확정) | step 1 | O 판매자는 확정 전까지 못 받음, 수수료 5% 차감 |
| 2 | 만료 실효 | step 4-3 | O 스케줄러가 티켓 3건 EXPIRED, 알림 발송 |
| 3 | 예약금 몰수 | step 3·4 | O 10분 후 취소 / 제한시간 초과 둘 다 PLATFORM으로 |
| 4 | ~~부분성사~~ → **청약철회·판매자 철회** | step 2·5 | O 둘 다 전액 환불 (5단계 재설계로 대체) |
| 5 | 동시성 락 없음 vs 적용 | step 8 | O 5~9건 중복 → 1건, 그런데 원장은 내내 맞음 |
| 6 | **최종 정합성** | step 10 | O PASS |
...
인메모리 H2라 **한 세션 = 앱 새로 띄우고 `verify.sh` 완주**다. 스크립트가 [캡처] 표시를
